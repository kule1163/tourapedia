...
const GetByCategName = () => {
    const { categ } = (0, react_router_dom_1.useParams)();
    const dispatch = (0, hooks_1.useAppDispatch)();
    const filteredPosts = (0, hooks_1.useAppSelector)((state) => state.posts.entities);
    (0, react_1.useEffect)(() => {
        if (categ) {
            dispatch((0, asyncThunks_1.getPostByCateg)(categ));
        }
    }, [categ]);
    return (<FilteredPosts_1.default header={`Category: ${categ}`} filteredPosts={filteredPosts} text={`No tours by category: "${categ}"`}/>);
};
exports.default = GetByCategName;
